const StudyPlanner = (() => {
  ...
  function renderSessionList(sessions) {
    ...
    return sessions.sort((a, b) => a.startTime.localeCompare(b.startTime)).map(s => `
      <div class="planner-session ${s.completed?'completed':''}" data-id="${s.id}" style="display:flex;align-items:center;gap:14px;padding:12px 14px;border:1px solid var(--border-color);border-radius:var(--radius-md);margin-bottom:8px;background:var(--bg-card);transition:all var(--transition);${s.completed?'opacity:0.6;':''}">
